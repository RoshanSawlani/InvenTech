...
export default function OptionCard({ optionData }) {
    ...
    return (
        <div className="shadow-md bg-white flex flex-col items-center justify-center gap-4 p-6 rounded">
            ...
            {
                enabled ? (
                    <Link href={link} className='py-2 rounded-sm bg-blue-600 text-white inline-flex items-center px-3 space-x-2'>{linkTitle}</Link>
                ) : (<button className="py-2 rounded-sm bg-blue-600 text-white inline-flex items-center px-3 space-x-2">Enable</button>)
            }
